...
router.get("/todos/test", (req, res) => {
    return res.status(200).send("<h1>Todo rutes </h1>")
})
...
router.post("/todo", (req, res) => {
    ...
    newTodo
        .save()
        .then(todo => {
            res.status(200).json({ id: todo._id, title: todo.title, done: todo.done })
        })
        .catch(err => {
...
router.delete("/todo/:id", (req, res) => {
    TodoModel.findByIdAndRemove(req.params.id, (err, todo) => {
        if (err) {
            ...
            res.status(200).json({ message: "Succesfully deleted", id: todo._id })
        } else {
